...
struct	tcb{ 
	...
	struct tcb	*nextTCB;		//	linked-list	pointer to next item
	uint32_t status; //	status options: active=0, dead=1, asleep=2
	struct tcb *prevTCB; //linked-list pointer to previous item
	uint32_t tid; //	thread ID
	uint32_t sleep_counter; //	keeps track of the timeslice
	uint32_t sleep_time; // keeps track of when it was called
}; 
