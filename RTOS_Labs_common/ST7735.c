...
void ST7735_Message(uint32_t  d, uint32_t  l, char *pt, int32_t value){
  ...
  if(d == 0) {
	...
	OS_bWait(&printSema);
	ST7735_FillRect(0, ypos*10, ST7735_TFTWIDTH, 10, ST7735_BLACK);
  // Check if line is within bounds
  if(l > 7) return;
  ST7735_SetCursor(0, ypos);
  ST7735_OutString(pt);
	if(value!=NULL){
		snprintf(valueStr, sizeof(valueStr), "%d", value);
		ST7735_OutString(valueStr);
	}
	OS_bSignal(&printSema);
