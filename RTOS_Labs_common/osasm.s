...
;Students will implement these functions as part of EE445M/EE380L.12 Lab
		...
		EXTERN	SleepNextRunPt	 ; stored next tcb when killed or asleep
...
; put your code here
    LDR R0, =RunPt
	LDR R1, [R0]
	LDR SP, [R1]  ;load localSP
	POP {R4-R11}
	POP {R0-R3}
	POP {R12}
	POP {LR} ;discard old LR
	POP {LR} ;LR = PC on stack that points to what we want to run 
	POP {R2} ;discard PSR
...
; put your code here
	...
	PUSH{R4-R11}
	LDR R0, =RunPt
	LDR R1, [R0] ;R1 points to currently running thread
	LDR R2, [R1, #8] ; get the thread flag
	STR SP, [R1]; save the stack pointer
	CMP R2, #0
	BEQ RegularSwitch
	B KillSwitch
...
RegularSwitch	
	LDR R1, [R1, #4] ;load next thread pt
	LDR SP, [R1]; load next stack pointer position into sp register
	
	STR R1, [R0]; store next thread pointer into runpt
	B EndHandler

KillSwitch
	; grab the next tcb from the runpt and 
	LDR R2, =SleepNextRunPt ;load next thread pt
	LDR R1, [R2]
	LDR R2, [R1, #4]
	LDR SP, [R2]; store next stack pointer position into sp register
	
	STR R2, [R0]; store next thread pointer into runpt
	B EndHandler
